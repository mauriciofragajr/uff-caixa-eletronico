<%@page import="caixaeletronico.utils.StringUtil"%>
<%@page import="java.util.List"%>
<%@page import="caixaeletronico.models.Movimentacao"%>
<%@page contentType="text/html" pageEncoding="UTF-8"%>

<!doctype html>
<html lang="pt-br">
    <head>
        <meta charset="utf-8">
        <meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">
        <meta name="description" content="">
        <meta name="author" content="">
        <link rel="icon" href="data:image/jpeg;base64,">

        <title>Extrato</title>

        <!-- Bootstrap core CSS -->
        <link href="http://getbootstrap.com/docs/4.1/dist/css/bootstrap.min.css" rel="stylesheet">

        <style>
            /*
     * Globals
     */

            /* Links */
            a,
            a:focus,
            a:hover {
                color: #fff;
            }

            /* Custom default button */
            .btn-secondary,
            .btn-secondary:hover,
            .btn-secondary:focus {
                color: #333;
                text-shadow: none; /* Prevent inheritance from `body` */
                background-color: #fff;
                border: .05rem solid #fff;
            }
            .btn {
                width: 200px;
                margin: 10px;
            }

            /*
             * Base structure
             */

            html,
            body {
                height: 100%;
                background-color: #008080;
            }

            body {
                display: -ms-flexbox;
                display: flex;
                color: #fff;
                text-shadow: 0 .05rem .1rem rgba(0, 0, 0, .5);
                box-shadow: inset 0 0 5rem rgba(0, 0, 0, .5);
            }

            .cover-container {
                max-width: 42em;
            }


            /*
             * Header
             */
            .masthead {
                margin-bottom: 2rem;
            }

            .masthead-brand {
                margin-bottom: 0;
            }

            .nav-masthead .nav-link {
                padding: .25rem 0;
                font-weight: 700;
                color: rgba(255, 255, 255, .5);
                background-color: transparent;
                border-bottom: .25rem solid transparent;
            }

            .nav-masthead .nav-link:hover,
            .nav-masthead .nav-link:focus {
                border-bottom-color: rgba(255, 255, 255, .25);
            }

            .nav-masthead .nav-link + .nav-link {
                margin-left: 1rem;
            }

            .nav-masthead .active {
                color: #fff;
                border-bottom-color: #fff;
            }

            @media (min-width: 48em) {
                .masthead-brand {
                    float: left;
                }
                .nav-masthead {
                    float: right;
                }
            }

            /*
             * Cover
             */
            .cover {
                padding: 0 1.5rem;
            }
            .cover .btn-lg {
                padding: .75rem 1.25rem;
                font-weight: 700;
            }

            /*
             * Footer
             */
            .mastfoot {
                color: rgba(255, 255, 255, .5);
            }
        </style>
    </head>

    <body class="text-center">

        <div class="cover-container d-flex w-100 h-100 p-3 mx-auto flex-column">
            <jsp:include page="partials/header.jsp" />

            <main role="main" class="inner cover">
                <h1 class="cover-heading">Extrato</h1>

                <form method="POST">
                    <div class="row">
                        <div class="col">
                            <div class="form-group">
                                <label for="dataInicio" class="col-form-label">Data Início</label>
                                <input type="date" value="<%=(request.getAttribute("dataInicio"))%>" class="form-control" id="dataInicio" name="dataInicio" required>
                            </div>
                        </div>
                        <div class="col">
                            <div class="form-group">
                                <label for="dataFinal" class="col-form-label">Data Fim</label>
                                <input type="date" value="<%=(request.getAttribute("dataFinal"))%>" class="form-control" id="dataFinal" name="dataFinal" required>
                            </div>
                        </div>
                    </div>
                    <button class="btn btn-secondary" type="submit">Mudar</button>

                </form>

                <p class="lead">
                <ul class="list-group mb-3">

                    <% for (Movimentacao mov : (List<Movimentacao>) request.getAttribute("extrato")) {%>
                    <li class="list-group-item d-flex justify-content-between bg-light">
                        <span class="text-dark"><%= StringUtil.dateToString(mov.getCriadoEm())%></span>
                        <%
                            String cpfLogado = (String) session.getAttribute("cpfLogado");
                            String color = "dark";
                            String tipoMovimentacao = mov.getTipo();

                            if (tipoMovimentacao.equals("DEPOSITO")) {
                                color = "success";
                            } else if (tipoMovimentacao.equals("SAQUE")) {
                                color = "danger";
                            } else if (tipoMovimentacao.equals("TRANSFERENCIA")) {
                                if (cpfLogado.equals(mov.getContaOrigem())) {
                                    color = "danger";
                                } else if (cpfLogado.equals(mov.getContaDestino())) {
                                    color = "success";
                                }
                            }
                        %>
                        <div class="text-dark">
                            <h6 class="my-0"><%=mov.getTipo()%> 
                                <% if ("TRANSFERENCIA".equals(tipoMovimentacao)) {%>
                                <%=(cpfLogado.equals(mov.getContaOrigem()) ? "Para " + mov.getContaDestino() : "De " + mov.getContaOrigem())%>
                                <% }%>
                            </h6>
                        </div>
                        <span class="text-<%=color%>" >
                            R$<%=(StringUtil.centavoToReal(mov.getValor()))%>
                        </span>
                    </li>
                    <% }%>

                    <li class="list-group-item d-flex justify-content-between">
                        <span class="text-dark">Total (R$)</span>
                        <strong class="text-dark">R$ <%=StringUtil.centavoToReal(request.getAttribute("total").toString())%></strong>
                    </li>
                </ul>
                </p>
            </main>
            <footer class="mastfoot mt-auto">
                <div class="inner">
                    <p><b>Banco - </b><%= new java.util.Date()%></p>
                </div>
            </footer>
        </div>


    </body>
</html>

