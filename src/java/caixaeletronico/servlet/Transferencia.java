...
public class Transferencia extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        String cpfLogado = (String) session.getAttribute("usuarioLogado");

        if (StringUtil.isEmpty(cpfLogado)) {
            response.sendRedirect(request.getContextPath() + "/Login");
        } else {
            RequestDispatcher resposta = request.getRequestDispatcher("WEB-INF/pages/menu/transferencia.jsp");
